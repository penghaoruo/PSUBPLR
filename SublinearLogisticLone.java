...
class DualMap extends Mapper<LongWritable, Text, IntWritable, Text> {
	...
	protected void map(LongWritable key, Text value, Context context) throws IOException,InterruptedException {
		...
		double sigma=x_value*len*len/w[jt]+b*y;
...
// Main Class for Sublinear Logistic Regression with l1-penalty
public class SublinearLogisticLone {
	...
	// Value of w[] in previous iteration
	static double prevw[]; 
	...
	static public void wUpdate(int k) throws Exception {
		...
		double tmp;
		int index;
		String uri = "sublinear/tmp/primal"+(new Integer(k)).toString()+"/part-r-00000";
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(URI.create(uri), conf);
		FSDataInputStream in = null;
		try {
		...
		if (k!=0)
			for (int i=0;i<d;i++) {
				if (prevw[i]>0 && w[i]>0)
					w[i]=Math.max(w[i]-gamma, 0);
				if (prevw[i]<0 && w[i]<0)
					w[i]=Math.min(w[i]+gamma, 0);
			}
	...
	public static void main(String[] args)throws Exception {
		...
		for (int k=0;k<T;k++) {
			...
			job_primal.setJarByClass(SublinearLogisticLone.class);
			...
			job_primal.setNumReduceTasks(1);
			...
			job_dual.setJarByClass(SublinearLogisticLone.class);
